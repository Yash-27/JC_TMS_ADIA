...
# plot_try.jl
#
# Self-contained plotting for the (β, x) sweep. Reads results.jld2, produces
# 2D maps of each observable, saves PDF into figures/.
#
# Run it whole, or step through it line by line at the prompt. Everything is
# at top level, so `d`, `p`, etc. stay available in the session afterwards.
#
...
# ==============================================================================
# RUN -- top level, so d / figs stay in the session afterwards
# ==============================================================================

apply_theme!()

d = load_results("results.jld2")          # or load_results("some_other_run.jld2")
...
for key in (:tracedist, :concurrence_full, :concurrence_adia, :concurrence_diff)
    if haskey(d.outs, key)
        figs[key] = plot_map(d, key)
        save_fig(figs[key], string(key))
    end
end

println("\ndone -- figures in figures/")
println("`d` and `figs` are available; e.g. figs[:tracedist], or")
println("plot_map(d, :tracedist; markersize = 10, show_params = false)")